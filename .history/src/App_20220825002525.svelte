<script>
  ...
  import { createClient } from '@urql/svelte';
	...
	const client = createClient({
		url: 'https://d33ip4f514s3iy.cloudfront.net/cms/read/en-US',
		fetchOptAions: () => {
			const token = 'a65bc94d4d3ae90711af1c46491d33c2dbe2a8ff15c9c8f2';
			return {
...
{:else}
	<ul>
		{#each $todos.data.listBlogs.data as todo}
		   {console.log($todos.data.listBlogs.meta)}
			<li>{todo.name}</li>
			<li>{todo.content}</li>
		{/each}
	</ul>
